...
for movie_num=1:num_of_movies
    ...
    for yL=1:y_cyc*(movies_info(movie_num).Height/Resol_y)   %movies_info(movie_num).Height
        %         lin
        for xL=1:x_cyc*(movies_info(movie_num).Width/Resol_x)  %movies_info(movie_num).Width
            for tL=1:t_cyc*(movies_info(movie_num).NumFrames/Resol_t)  %movies_info(movie_num).NumFrames
    ...
    for y=1:y_cyc*(movies_info(movie_num).Height/Resol_y)   %base_block_size(4)
        for x=1:x_cyc*(movies_info(movie_num).Width/Resol_x)%  x_cyc   %WAS x_cyc instead put: number of low res points in the base block
                % YUV - x is in the low space. should not run up to x_cyc, but from 1 to 2 (=?? length(base_block_size(3)))
            num_of_copies=Resol_t/t_cyc;  % to duplicate the t_cycle
%%%%%            remainder=mod(movies_info(movie_num).NumFrames,t_cyc)   % If it's not a whole number
            
            dist=t_cyc;  % shifting to the right of the duplicated blocks
            ...
            index_fix=kron(transpose([0:num_of_copies-1]),repmat([ t_cyc* (movies_info(movie_num).NumFrames/Resol_t) dist 0],size(block_to_copy,1),1));
...
%%%%%            movie_A{movie_num,y}=[movie_A{movie_num,y}; temp_A( 1 : (size(temp_A,1)-num_of_lines_to_remove) , : )];
            movie_A{movie_num,y}=[movie_A{movie_num,y}; temp_A];
        end
...
%%%%%    lines_ind_remove=[];
%     ((num_of_copies-1)*y_cyc+remainder)*Resol_t*Resol_y+1
%     num_of_copies*y_cyc*Resol_t*Resol_x
%%%%%    lines_ind_remove=[lines_ind_remove ~~remainder*( [ ((num_of_copies-1)*y_cyc+remainder)*Resol_t*Resol_x+1:num_of_copies*y_cyc*Resol_t*Resol_x])];
