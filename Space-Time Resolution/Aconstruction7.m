function [ A ] = Aconstruction( movies_names,Resol_x,Resol_y,Resol_t,T_Mat,input)
%   ACONSTRUCTION constructs the matrix representing the system of equations.

[num_of_movies movies_info]=GetMoviesInfo(movies_names);

% Resolution increase factors (for each axis)
...
for movie_num=1:num_of_movies
    ...
    dx=1/ResolRatioX(movie_num);    % interval size representing the transformation between axes.
    dy=1/ResolRatioY(movie_num);
    dt=1/ResolRatioT(movie_num);
    ...
    Supports=Bi(0,0,0,movie_num,1,input);
    ...
    for yL=1:y_cycL
        for xL=1:x_cycL
            for tL=1:t_cycL
                ...
                min_diff_from_min_x=0;
                min_diff_from_max_x=0;
                min_diff_from_min_y=0;
                min_diff_from_max_y=0;
                min_diff_from_min_t=0;
                min_diff_from_max_t=0;                
                
                % we search for non-zero elements of A within the area of the support around the given pL
                
                for y=y0-sup_y-1:y0+sup_y+1
                    
                    [ interval_yH ] = IntervalCalc( y,sup_yL_maxH,sup_yL_minH,dy );
                    
                    for x=x0-sup_x-1:x0+sup_x+1

                        [ interval_xH ] = IntervalCalc( x,sup_xL_maxH,sup_xL_minH,dx );

                        for t=t0-sup_t-1:t0+sup_t+1
                            [ interval_tH ] = IntervalCalc( t,sup_tL_maxH,sup_tL_minH,dt );
                            ...
                            val=Bi(point_for_Bi(1),point_for_Bi(2),point_for_Bi(3),movie_num,0,input);
    ...
    for yL=1:y_cycL
        for xL=1:x_cycL
            num_of_copies=Resol_t/t_cyc;  % the number of copies that shall be replicated along the t direction
                                          ...
                                          % unless the numerator and denominator of Resol_z/size_of_z_axis have
                                          % common divider, the cycle will be Resol_z, and thus num_of_copies will
                                          % equal 1. otherwise, the cycle will be the numerator after reduction,
                                          % and num_of_copies will still be a whole number.
    ...
    temp_A=Mikrei_Katze(temp_A,Resol_x,Resol_y,Resol_t,T_Mat,movies_info,movie_num,input);
